...
int yylval_token (int symbol) {
   ...
   fprintf(tok_file_out,"%-4zu%3d.%-5d%-5d%-16s(%s)\n"
                       , included_filenames.size(), scan_linenr
                       , offset, symbol, get_yytname(symbol), yytext);
   return symbol;
}
...
void scanner_include (void) {
   ...
   if (scan_rc != 2) {
      ...
      // prints the included file names
      fprintf (tok_file_out,"# %d \"%s\"\n", linenr, filename);
